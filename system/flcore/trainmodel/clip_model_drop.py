...
class LinearWithLoRA(nn.Module):
    ...
    def forward(self, x):
        if self.lora is not None and self.lora.rank > 0: # Check if LoRA is available
            return self.linear(x) + self.lora(x)
        else:
            return self.linear(x)  # No contribution from LoRA if rank = 0 or fallback to the original linear layer if LoRA is removed
        # return self.linear(x) + self.lora(x)
...
class CLIPModelWithLoRA(torch.nn.Module):
    ...
    def get_number_of_lora_layers(self):
        """
        Get the number of layers in the CLIP model that can potentially use LoRA.
        """
    ...
    def set_lora_dict_select(self, dictionary):
        """
        Set the parameters of the LoRA layers from a selected dictionary.
        ...
        for key, param in dictionary.items():
            if key in self.lora_layers:
                self.lora_layers[key].data.copy_(param.data)
            else:
                print(f"LoRA layer {key} not found in the model.")
