...
class GatingMoE(nn.Module):
    def __init__(self, input_dim, moe_hidden_scale):
        ...
        self.gate = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim, 2),  # Output 2 weights: one for each expert
...
class LinearWithLoRACombinedMoE(nn.Module):
    def __init__(self, 
        ...
        # Gating network for MoE (takes in the input features)
        self.gating_network = GatingMoE(input_dim=linear.in_features, moe_hidden_scale=self.moe_hidden_scale)
    ...
    def forward(self, x):
        ...
        # Apply global and local LoRA if ranks > 0
        if self.lora_global.rank > 0:
            global_output = self.lora_global(x)
        else:
            global_output = torch.zeros_like(base_output)

        if self.lora_local.rank > 0:
            local_output = self.lora_local(x)
        else:
            local_output = torch.zeros_like(base_output)
            
        # Combine outputs using gating weights
        combined_output = base_output + alpha_global * global_output + alpha_local * local_output
        return combined_output
    
    
class CLIPModelWithLoRACombinedMoE(torch.nn.Module):
    def __init__(self, model_checkpoint, home_dir, lora_params_global, lora_params_local, moe_params, momentum_global=0.1, momentum_local=0.5):
        """
