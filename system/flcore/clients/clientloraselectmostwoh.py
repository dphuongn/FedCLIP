...
class clientLORASELECTMOSTWOH(Client):
    ...
    def prepare_most_important_layers(self, sorted_layers):
        """Prepare the most important LoRA layers to be sent to the server."""
        ...
        for layer_idx in most_important_indices:
            ...
            for submodule_name, submodule in encoder_layer.named_modules():
                if isinstance(submodule, LinearWithLoRA):
                    ...
                    most_important_lora_layers[f"{full_layer_name}.W_a"] = submodule.lora.W_a
                    most_important_lora_layers[f"{full_layer_name}.W_b"] = submodule.lora.W_b

        # Handle projection layers
        # if self.lora_params.get('lora_head_text', False):
        #     most_important_lora_layers["text_projection.W_a"] = self.lora_layers['text_projection.W_a']
        #     most_important_lora_layers["text_projection.W_b"] = self.lora_layers['text_projection.W_b']

        # if self.lora_params.get('lora_head_vision', False):
        #     most_important_lora_layers["visual_projection.W_a"] = self.lora_layers['visual_projection.W_a']
        #     most_important_lora_layers["visual_projection.W_b"] = self.lora_layers['visual_projection.W_b']

        return most_important_lora_layers

    def send_most_important_layers(self, sorted_layers):
        """Send only the most important LoRA layers to the server."""
        most_important_layers = self.prepare_most_important_layers(sorted_layers)
        self.lora_layers_select = most_important_layers
        print("Most important LoRA layers sent to server:")
        for key in most_important_layers.keys():
            print(f"{key}")
