...
class clientLORASELECTRANDOM(Client):
    ...
    def prepare_random_k_lora_layers(self):
        """Prepare random k LoRA layers, including projection layers."""
        all_lora_layers = {}

        # Collect LoRA layers from text encoder
        for i, layer in enumerate(self.clip_model.text_model.encoder.layers):
            ...
            for submodule_name, submodule in layer.named_modules():
                if isinstance(submodule, LinearWithLoRA):
                    ...
                    all_lora_layers[f"{full_layer_name}.W_a"] = submodule.lora.W_a
                    all_lora_layers[f"{full_layer_name}.W_b"] = submodule.lora.W_b

        # Collect LoRA layers from vision encoder
        ...
        if "text_projection.W_a" in self.lora_layers:
            all_lora_layers["text_projection.W_a"] = self.lora_layers["text_projection.W_a"]
            all_lora_layers["text_projection.W_b"] = self.lora_layers["text_projection.W_b"]
        
        if "visual_projection.W_a" in self.lora_layers:
            all_lora_layers["visual_projection.W_a"] = self.lora_layers["visual_projection.W_a"]
            all_lora_layers["visual_projection.W_b"] = self.lora_layers["visual_projection.W_b"]
        
        # Select random k LoRA layers
        selected_layers = dict(random.sample(list(all_lora_layers.items()), min(self.k_layers * 2, len(all_lora_layers))))
        return selected_layers

    def send_random_k_lora_layers(self):
        """Send k random LoRA layers, including projection layers, to the server."""
        self.lora_layers_select = self.prepare_random_k_lora_layers()
        print("Random k LoRA layers sent to server:")
