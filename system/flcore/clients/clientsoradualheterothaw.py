...
class clientSORADUALHETEROTHAW(Client):
        ...
        for epoch in range(self.local_epochs):
            ...
            with tqdm(trainloader, total=len(trainloader)) as pbar:  # Initialize pbar here
                for i, batch in enumerate(pbar):
                    ...
                    kl_div_loss_image = self.kl_loss(logits_per_image_local, logits_per_image_global)
                    kl_div_loss_text = self.kl_loss(logits_per_text_local, logits_per_text_global)
                    ...
                    total_loss = cross_entropy_loss + self.gamma_local*kl_div_loss
                    
                    # total_loss = (1 - self.mu_local) * cross_entropy_loss + self.mu_local * kl_div_loss

                    # Accumulate batch loss
                    epoch_cross_entropy_loss += cross_entropy_loss.item() * target.size(0)
                    epoch_kl_div_loss += kl_div_loss.item() * target.size(0)
                    epoch_total_loss += total_loss.item() * target.size(0)
                    total_samples += target.size(0)

                    
                    # Optimize both Client SoRA and global LoRA
                    # Backward pass
                    ...
                    # end update both Adapters -------------------------------------------------------
                    
                    train_num += target.size(0)


            # After the epoch, compute the average loss for the entire epoch
            avg_cross_entropy_loss = epoch_cross_entropy_loss / total_samples
            avg_kl_div_loss = epoch_kl_div_loss / total_samples
            avg_total_loss = epoch_total_loss / total_samples
            
            # Print the average loss for the epoch
            print(f"Epoch {epoch+1}/{self.local_epochs} for client {self.id}:")
            print(f"Avg CE Loss: {avg_cross_entropy_loss:.4f}, Avg KL Loss: {avg_kl_div_loss:.4f}, Avg Total Loss: {avg_total_loss:.4f}")
