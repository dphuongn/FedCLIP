...
class clientLORASELECTHEAD(Client):
    ...
    def prepare_projection_layers(self):
        """Prepare the projection LoRA layers to be sent to the server."""

        projection_lora_layers = {}

        # Handle projection layers
        if self.lora_params.get('lora_head_text', False):
            projection_lora_layers["text_projection.W_a"] = self.lora_layers['text_projection.W_a']
            projection_lora_layers["text_projection.W_b"] = self.lora_layers['text_projection.W_b']

        if self.lora_params.get('lora_head_vision', False):
            projection_lora_layers["visual_projection.W_a"] = self.lora_layers['visual_projection.W_a']
            projection_lora_layers["visual_projection.W_b"] = self.lora_layers['visual_projection.W_b']

        return projection_lora_layers

    def send_projection_layers(self):
        """Send only the head LoRA layers to the server."""
        projection_layers = self.prepare_projection_layers()
        self.lora_layers_select = projection_layers
        print("Least projection LoRA layers sent to server:")
        for key in projection_layers.keys():
            print(f"{key}")
