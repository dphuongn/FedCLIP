...
import copy
import random
from flcore.clients.clientloradropreverse import clientLORADROPREVERSE
...
class FLoraDropReverse(Server):
    def __init__(self, args, times):
        ...
        self.set_clients(clientLORADROPREVERSE)
    ...
    def initialize_client_layers(self):
        """Calculate and remove most important layers for all clients before the first iteration."""
        print("\nInitializing clients with layer importance evaluation...")
        for client in self.clients:
            trainloader = client.load_train_data()
            client.clip_model.to(client.device)  # Move model to appropriate device
            client.clip_model.train()  # Set model to training mode

            sorted_layers = client.calculate_layer_importance(trainloader)
            client.remove_most_important_layers(sorted_layers)
            ...
            if not self.pfl: # tfl
                ...
                if i < self.global_rounds: # skip training for the last round to save time
                    ...
                    for client in self.selected_clients:
                        ...
                        # check W_b dimension
                        # for name, param in self.lora_layers.items():
                        #     print(f"Before training for client {client.id}: {name}, W_b size: {param.size()}")

                        client.train()

                        # check W_b dimension
                        # for name, param in self.lora_layers.items():
                        #     print(f"After training for client {client.id}: {name}, W_b size: {param.size()}")
    ...
    #         client.set_parameters(self.global_model)  # Send the global LoRA parameters
    ...
    def send_models(self):
        ...
        for client in self.clients:
            ...
            filtered_global_model = {
                ...
                if key in client.lora_layers
            }

            client.set_parameters(filtered_global_model)  # Send filtered LoRA parameters
            client.send_time_cost['num_rounds'] += 1
            client.send_time_cost['total_cost'] += 2 * (time.time() - start_time)
