...
import copy
import random
from flcore.clients.clientloradattruemoelocal import clientLORADATTRUEMOELOCAL
from flcore.servers.serverbase import Server
from threading import Thread
import statistics
import torch

from flcore.trainmodel.clip_model_moelocal import *


class FLoraDATTrueMoELocal(Server):
    def __init__(self, args, times):
        super().__init__(args, times)
        
        self.lora_params = args.lora_params

        self.moe_params = args.moe_params

        self.num_local_experts = args.num_local_experts
        
        self.clip_model_object = CLIPModelWithLoRACombinedMoE(model_checkpoint=args.model_checkpoint, home_dir=args.home_dir, 
                                                                lora_params_global=self.lora_params, lora_params_local=self.lora_params, 
                                                                moe_params=self.moe_params, num_local_experts=self.num_local_experts)
        
        self.global_model = copy.deepcopy(self.clip_model_object.lora_layers_global)    # dict of params
        
        self.set_clients(clientLORADATTRUEMOELOCAL)
    ...
    def train(self):
        ...
        if self.num_new_clients > 0:
            ...
            self.set_new_clients(clientLORADATTRUEMOELOCAL)
