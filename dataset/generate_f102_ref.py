...
dir_path = Path("flowers_ref")
num_classes = 102

# Allocate data to users
def generate_flowers(dir_path, num_clients, num_classes, niid, balance, partition, alpha, few_shot, n_shot, pfl):
    ...
    valset = torchvision.datasets.Flowers102(
        root=dir_path / "rawdata", split='val', download=True, transform=transform)
    ...
    val_images, val_labels = process_dataset(valset)
    ...
    print(f'val_images length: {len(val_images)}')
    ...
    dataset_image = np.concatenate((train_images, val_images, test_images), axis=0)
    dataset_label = np.concatenate((train_labels, val_labels, test_labels), axis=0)
...
if __name__ == "__main__":
    ...
    generate_flowers(dir_path, num_clients, num_classes, niid, balance, partition, alpha, few_shot, n_shot, pfl)