...
def plot_multiple_accuracy_curves(filenames, labels, colors):
    ...
    plt.savefig(f'{dataset}_{partition}_{alg}_{r}_{rl}_{a}_{sl}_{gl}_all_pfl.pdf', format='pdf', bbox_inches='tight')

    plt.close()

lrs = ['5e-5', '1e-5', '5e-6', '1e-6']
# lrs = ['1e-5', '5e-6', '1e-6']      # test fsoradualheterov 10
# lrs = ['5e-6', '1e-6']
# lrs = ['5e-6']
# lrs = ['1e-6']
# lrs = ['1e-5']
...
# lrs = ['1e-5']                    # fsoradualheterov
# lrs = ['5e-5']                    # fsoradualheterot


# gammas_local = ['0.001']          # fsoradualheterov no
# gammas_local = ['0.01']           # fsoradualheterov
# gammas_local = ['0.1']            # fsoradualheterov
# gammas_local = ['1']              # fsoradualheterot
gammas_local = ['10']             # fsoradualheterot
            ...
            for wd in wds:
                for r in ranks:
                    for rl in ranks_local:
                        for a in alphas:
                            filenames.append(f'../../logs/{dataset}/{dataset}_{partition}_{alg}_lr{lr}_wd{wd}_r{r}_rl{rl}_a{a}_sl{sl}_gl{gl}_all_pfl.out')
                            labels.append(f'lr={lr}, wd={wd}, r={r}, rl={rl}, a={a}, sl={sl}, gl={gl}')
