...
alg = 'floradattruemoelocalv'
# alg = 'floradattruemoelocalt'
# alg = 'floradattruemoe2'
...
def plot_multiple_accuracy_curves(filenames, labels, colors):
    ...
    plt.savefig(f'{dataset}_{partition}_{alg}_{r}_{a}_{hs}_{nle}_all_pfl.pdf', format='pdf', bbox_inches='tight')

    plt.close()

lrs = ['5e-5', '1e-5', '5e-6', '1e-6']

# lrs = ['1e-5', '5e-6']                   # dir floradattruev
# lrs = ['5e-5', '1e-5', '5e-6']           # dir floradattruet

# lrs = ['1e-5', '5e-6']                   # dir10 floradattruev
# lrs = ['1e-5', '5e-6']                   # dir10 floradattruet

# lrs = ['1e-5', '5e-6']                   # dir001 floradattruev
# lrs = ['5e-5', '1e-5', '5e-6']           # dir001 floradattruet

# moe_hidden_scales = ['0.5']
# moe_hidden_scales = ['1']
moe_hidden_scales = ['2']
# moe_hidden_scales = ['4']
...
for hs in moe_hidden_scales:
    for nle in num_local_experts:
        for lr in lrs:
            for wd in wds:
                for r in ranks:
                    for a in alphas:
                        filenames.append(f'../../logs/{dataset}/{dataset}_{partition}_{alg}_lr{lr}_wd{wd}_r{r}_a{a}_hs{hs}_nle{nle}_all_pfl.out')
                        labels.append(f'lr={lr}, wd={wd}, r={r}, a={a}, hs={hs}, nle={nle}')
