                ...
                for a in "${alphas[@]}"; do
                    for lr in "${learning_rates[@]}"; do
                        for wd in "${weight_decays[@]}"; do
                            job_name="${dataset}_${partition}_${algo}v_lr${lr}_wd${wd}_r${r}_rl${rl}_a${a}_sl${sl}_gl${gl}_all_pfl"
                            output_file="${log_dir}/${job_name}.out"
                            error_file="${log_dir}/${job_name}.err"

                            echo "$PWD"
                            echo "Running with algo=${algo}, lr=${lr}, wd=${wd}, r=${r}, a=${a}, sl=${sl}, rl=${rl}, gl=${gl}"
                            ...
                            echo "Submitted job $job_name with algo=${algo}, lr=${lr}, wd=${wd}, r=${r}, a=${a}, sl=${sl}, rl=${rl}, gl=${gl} at $(date)"
                        done
                    done
                done
            done
