...
def run_script(lr, i_s, m_i_l, g_t, m_s_f, seed, device):
    """Runs the script with assigned parameters on a given GPU"""
    global gpu_usage

    job_name = f"{dataset}_{partition}_{algo}t_lr{lr}_is{i_s}_mil{m_i_l}_gt{g_t}_msf{m_s_f}_sd{seed}_all_pfl"
    ...
    command = [
        ...
        "-lr", str(lr),
        "-inner_steps", str(i_s),
        "-meta_inner_lr", str(m_i_l),
        "-gumbel_temp", str(g_t),
        "-meta_support_fraction", str(m_s_f),
        "--lora_rank", "2",
        "--lora_alpha", "16",
        "--lora_key_text",
	    "--lora_query_text",
...
for params in itertools.product(learning_rates, inner_steps, meta_inner_lr, gumbel_temp, meta_support_fraction, seeds):
    lr, i_s, m_i_l, g_t, m_s_f, seed = params
    ...
    thread = threading.Thread(target=run_script, args=(lr, i_s, m_i_l, g_t, m_s_f, seed, device))
    thread.start()
