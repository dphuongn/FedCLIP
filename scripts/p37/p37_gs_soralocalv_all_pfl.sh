        ...
        for a in "${alphas[@]}"; do
            for lr in "${learning_rates[@]}"; do
                for wd in "${weight_decays[@]}"; do
                    job_name="${dataset}_${partition}_${algo}v_lr${lr}_wd${wd}_r${r}_a${a}_sl${sl}_all_pfl"
                    output_file="${log_dir}/${job_name}.out"
                    error_file="${log_dir}/${job_name}.err"

                    echo "$PWD"
                    echo "Running with algo=${algo}, lr=${lr}, wd=${wd}, r=${r}, a=${a}, sl=${sl}"
                    ...
                    echo "Submitted job $job_name with algo=${algo}, lr=${lr}, wd=${wd}, r=${r}, a=${a}, sl=${sl} at $(date)"
                done
            done
        done
    done
