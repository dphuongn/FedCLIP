        ...
        for a in "${alphas[@]}"; do
            for lr in "${learning_rates[@]}"; do
                for wd in "${weight_decays[@]}"; do
                    job_name="${dataset}_${partition}_${algo}t_lr${lr}_sd${sd}_all_pfl"
                    output_file="${log_dir}/${job_name}.out"
                    error_file="${log_dir}/${job_name}.err"

                    echo "$PWD"
                    echo "Running with algo=${algo}, lr=${lr}, sd=${sd}"
                    ...
                    echo "Submitted job $job_name with algo=${algo}, lr=${lr}, sd=${sd} at $(date)"
                done
            done
        done
    done
