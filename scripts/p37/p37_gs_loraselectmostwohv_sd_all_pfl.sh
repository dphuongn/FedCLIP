...
algo='floraselectmostwoh'

k_layers=(10)
# k_layers=(1 2 3 9 10 11)
...
# seeds=(1 42)
seeds=(0)
# seeds=(0 1 42)         # dir001 1e-5

for sd in "${seeds[@]}"; do
    for k in "${k_layers[@]}"; do
        for r in "${ranks[@]}"; do
            for a in "${alphas[@]}"; do
                for lr in "${learning_rates[@]}"; do
                    for wd in "${weight_decays[@]}"; do
                        job_name="${dataset}_${partition}_${algo}v_lr${lr}_wd${wd}_r${r}_a${a}_k${k}_sd${sd}_all_pfl"
                        output_file="${log_dir}/${job_name}.out"
                        error_file="${log_dir}/${job_name}.err"

                        echo "$PWD"
                        echo "Running with algo=${algo}, lr=${lr}, wd=${wd}, r=${r}, a=${a}, k=${k}, sd=${sd}"
                        ...
                        echo "Submitted job $job_name with algo=${algo}, lr=${lr}, wd=${wd}, r=${r}, a=${a}, k=${k}, sd=${sd} at $(date)"
                    done
                done
            done
        done
