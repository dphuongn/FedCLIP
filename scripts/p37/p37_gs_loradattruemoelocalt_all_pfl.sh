...
for hs in "${moe_hidden_scales[@]}"; do
    for nle in "${num_local_experts[@]}"; do
        for r in "${ranks[@]}"; do
            for a in "${alphas[@]}"; do
                for lr in "${learning_rates[@]}"; do
                    for wd in "${weight_decays[@]}"; do
                        job_name="${dataset}_${partition}_${algo}t_lr${lr}_wd${wd}_r${r}_a${a}_hs${hs}_nle${nle}_all_pfl"
                        output_file="${log_dir}/${job_name}.out"
                        error_file="${log_dir}/${job_name}.err"
        
                        echo "$PWD"
                        echo "Running with algo=${algo}, lr=${lr}, wd=${wd}, r=${r}, a=${a}, hs=${hs}, nle=${nle}"
                        ...
                        echo "Submitted job $job_name with algo=${algo}, lr=${lr}, wd=${wd}, r=${r}, a=${a}, hs=${hs}, nle=${nle} at $(date)"
                    done
                done
            done
        done
