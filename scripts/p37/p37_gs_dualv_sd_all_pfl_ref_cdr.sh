                    ...
                    for dt in "${distill_temp[@]}"; do
                        for rf in "${ref_data_fraction[@]}"; do
                            for cdr in "${client_drop_rate[@]}"; do
                                job_name="${dataset}_${partition}_${algo}v_lr${lr}_rbs${rbs}_dlr${dlr}_de${de}_dt${dt}_rf${rf}_sd${sd}_nc${nc}_all_pfl_ref_cdr${cdr}"
                                ...
                                echo "Running with algo=${algo}, lr=${lr}, rbs=${rbs}, dlr=${dlr}, de=${de}, dt=${dt}, rf=${rf}, sd=${sd}, nc=${nc}, cdr=${cdr}" | tee -a $output_file
