...
alg = 'floradattruemoev'
# alg = 'floradattruemoet'
...
def plot_multiple_accuracy_curves(filenames, labels, colors):
    ...
    plt.savefig(f'{dataset}_{partition}_{alg}_{r}_{a}_{hs}_all_pfl.pdf', format='pdf', bbox_inches='tight')
...
for hs in moe_hidden_scales:
    for lr in lrs:
        for wd in wds:
            for r in ranks:
                for a in alphas:
                    filenames.append(f'../../logs/{dataset}/{dataset}_{partition}_{alg}_lr{lr}_wd{wd}_r{r}_a{a}_hs{hs}_all_pfl.out')
                    labels.append(f'lr={lr}, wd={wd}, r={r}, a={a}, hs={hs}')
