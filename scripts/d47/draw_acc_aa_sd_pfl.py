...
def plot_multiple_accuracy_curves(filenames, labels, colors):
    ...
    plt.savefig(f'{dataset}_{partition}_{alg}_{r}_sd_pfl.pdf', format='pdf', bbox_inches='tight')
...
lrs = ['1e-3']                                # fedaav dir001
# lrs = ['1e-4']                                # fedaat dir001
...
for sd in seeds: 
    for lr in lrs:
        for wd in wds:
            for r in aa_b_rs:
                filenames.append(f'../../logs/{dataset}/{dataset}_{partition}_{alg}_lr{lr}_wd{wd}_r{r}_sd{sd}_pfl.out')
                labels.append(f'lr={lr}, wd={wd}, sd={sd}')
