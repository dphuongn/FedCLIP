                        ...
                        for lr in "${learning_rates[@]}"; do
                            for wd in "${weight_decays[@]}"; do
                                for cdr in "${client_drop_rate[@]}"; do
                                    job_name="${dataset}_${partition}_${algo}v_lr${lr}_wd${wd}_r${r}_rl${rl}_a${a}_sl${sl}_gl${gl}_sd${sd}_nc${nc}_all_pfl_cdr${cdr}"
                                    ...
                                    echo "Running with algo=${algo}, lr=${lr}, sl=${sl}, rl=${rl}, gl=${gl}, sd=${sd}, nc=${nc}, cdr=${cdr}" | tee -a $output_file
