...
def plot_multiple_accuracy_curves(filenames, labels, colors):
    ...
    plt.savefig(f'{dataset}_{partition}_{alg}_{r}_{a}_{sl}_all_pfl.pdf', format='pdf', bbox_inches='tight')
...
for sl in sls:
    for lr in lrs:
        for wd in wds:
            for r in ranks:
                for a in alphas:
                    filenames.append(f'../../logs/{dataset}/{dataset}_{partition}_{alg}_lr{lr}_wd{wd}_r{r}_a{a}_sl{sl}_all_pfl.out')
                    labels.append(f'lr={lr}, wd={wd}, r={r}, a={a}, sl={sl}')
