                            ...
                            for lr in "${learning_rates[@]}"; do
                                for wd in "${weight_decays[@]}"; do
                                    for cdr in "${client_drop_rate[@]}"; do
                                        job_name="${dataset}_${partition}_${algo}v_lr${lr}_wd${wd}_r${r}_rl${rl}_a${a}_sl${sl}_gl${gl}_gg${gg}_sd${sd}_nc${nc}_all_pfl_cdr${cdr}"
                                        output_file="${log_dir}/${job_name}.out"
                                        error_file="${log_dir}/${job_name}.err"

                                        echo "$PWD"
                                        echo "Running with algo=${algo}, lr=${lr}, wd=${wd}, r=${r}, a=${a}, sl=${sl}, rl=${rl}, gl=${gl}, sd=${sd}, gg=${gg}, nc=${nc}, cdr=${cdr}"
