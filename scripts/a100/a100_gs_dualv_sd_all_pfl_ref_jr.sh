                    ...
                    for dt in "${distill_temp[@]}"; do
                        for rf in "${ref_data_fraction[@]}"; do
                            for jr in "${join_ratio[@]}"; do
                                job_name="${dataset}_${partition}_${algo}v_lr${lr}_rbs${rbs}_dlr${dlr}_de${de}_dt${dt}_rf${rf}_sd${sd}_all_pfl_ref_jr${jr}"
                                ...
                                echo "Running with algo=${algo}, lr=${lr}, rbs=${rbs}, dlr=${dlr}, de=${de}, dt=${dt}, rf=${rf}, sd=${sd}, jr=${jr}" | tee -a $output_file
